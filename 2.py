...
root.geometry("800x600")  # Pencere boyutunu biraz büyüttüm
...
tk.Label(root, text="Başlangıç:", bg="#1e1e2e", fg="white", font=("Arial", 10)).grid(row=0, column=0, padx=10, pady=5, sticky="w")
start_datetime_entry = DateTimeEntry(root)
start_datetime_entry.grid(row=0, column=1, padx=10, pady=5)

tk.Label(root, text="Bitiş:", bg="#1e1e2e", fg="white", font=("Arial", 10)).grid(row=1, column=0, padx=10, pady=5, sticky="w")
end_datetime_entry = DateTimeEntry(root)
end_datetime_entry.grid(row=1, column=1, padx=10, pady=5)

# Kalan MB ve Toplam GB
tk.Label(root, text="Kalan MB:", bg="#1e1e2e", fg="white", font=("Arial", 10)).grid(row=2, column=0, padx=10, pady=5, sticky="w")
entry_remaining_mb = ttk.Entry(root)
entry_remaining_mb.grid(row=2, column=1, padx=10, pady=5)

tk.Label(root, text="Toplam GB:", bg="#1e1e2e", fg="white", font=("Arial", 10)).grid(row=3, column=0, padx=10, pady=5, sticky="w")
entry_total_gb = ttk.Entry(root)
entry_total_gb.grid(row=3, column=1, padx=10, pady=5)

# Sonuçlar
label_total_days = ttk.Label(root, text="Toplam Gün: --")
label_total_days.grid(row=4, column=0, columnspan=2, pady=10, sticky="w")

label_daily_mb = ttk.Label(root, text="Toplam Güne Göre (MB): --")
label_daily_mb.grid(row=5, column=0, columnspan=2, pady=10, sticky="w")

label_average_download = ttk.Label(root, text="Ortalama İndirme (MB/Gün): --", foreground="orange")
label_average_download.grid(row=6, column=0, columnspan=2, pady=10, sticky="w")

label_profit_mb = ttk.Label(root, text="Kâr (MB): --", foreground="green")
label_profit_mb.grid(row=7, column=0, columnspan=2, pady=10, sticky="w")

label_profit_days = ttk.Label(root, text="Kâr (Gün): --", foreground="lightblue")
label_profit_days.grid(row=8, column=0, columnspan=2, pady=10, sticky="w")

label_daily_limit = ttk.Label(root, text="Sınır (MB/Gün): --", foreground="purple")
label_daily_limit.grid(row=9, column=0, columnspan=2, pady=10, sticky="w")

label_time_passed = ttk.Label(root, text="Geçen Süre: --")
label_time_passed.grid(row=10, column=0, columnspan=2, pady=10, sticky="w")

label_time_remaining = ttk.Label(root, text="Kalan Süre: --")
